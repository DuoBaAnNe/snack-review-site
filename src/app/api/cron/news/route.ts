...
// AI intro writer — DeepSeek via its Anthropic-compatible gateway.
// The endpoint is the Anthropic Messages API (/anthropic/v1/messages),
// NOT the OpenAI /v1/chat/completions path.
...
async function aiIntro(title: string): Promise<string | null> {
...
新闻标题：${title}`;
    try {
        ...
        console.error('[ai-intro]', e?.message || e);
        return null;
    }
}
